...
def test_cell_generation_from_coordinates_at_one_level(self):
    ...
    assert len(cells) == 1
    rect = cells[0].bounds()
    assert rect[0] == -82.34518902474312
    assert rect[1] == 35.80182243257457
    assert rect[2] == -82.33976347120388
    assert rect[3] == 35.80759183314543


def test_constants(self):

    consts = constants.Constants()
    assert consts['standard_s2_cell_level'] == 14
    assert consts['max_coordinate_uncertainty_meters'] == 7000

    assert len(Cell.from_coordinates(
        lat=35.8021685,
        lng=-82.3451891,
