...
from yfeatures import Landcover
from yoccurrences import Occurrence
...
def test_landcover():
    ...
    assert sorted(records, key=lambda k: k['cell_id']) == sorted(expected_records, key=lambda k: k['cell_id'])

    assert [o.get_feature('landcover') for o in landcover_response.occurrences] == \
